...
def world_bank_data(filename):
    ...
    converted_df = df.set_index(
        ['Country Name', 'Indicator Name']).stack().unstack(0).reset_index()
...
# Bar Char Comparison
dataforbar = data[
    data['Indicator Name'] == 'Ease of doing business rank (1=most business-friendly regulations)'][
    ['Bahrain', 'Spain']]
...
plt.pie(top5_countries['Total'],
        labels=top5_countries['Country Name'],
        autopct='%1.1f%%', startangle=90)
plt.legend(
    bbox_to_anchor=(1, 0.5), loc="center left", title="Country Name",
...
selected_data = data[data['Indicator Name'] == indicator][['Pakistan']]
